 ...
 * @version $Id: LC_Page_Admin_System_Input.php 22504 2013-02-05 12:02:12Z m_uehara $
 */
class LC_Page_Admin_System_Input extends LC_Page_Admin_Ex {
    ...
    function initForm(&$objFormParam, &$arrParams, $mode = '') {
        ...
        if ($mode == 'edit' && $arrParams['password'] == DEFAULT_PASSWORD) {
            ...
            $objFormParam->addParam(t('c_Password (confirmation)_01'), 'password02', '' , '', array('EXIST_CHECK'));
        } else {
    ...
    function validateData(&$objFormParam, &$arrParams, $mode) {
        ...
        if ($mode == 'new') {
            $objErr->doFunc(array(t('c_Password_01'), 'password', ID_MIN_LEN, ID_MAX_LEN), array('NUM_RANGE_CHECK'));
            $objErr->doFunc(array(t('c_Login ID_01'), 'login_id', ID_MIN_LEN, ID_MAX_LEN), array('NUM_RANGE_CHECK'));
        } elseif ($mode == 'edit') {
            $objErr->doFunc(array(t('c_Password_01'), 'password', ID_MIN_LEN, ID_MAX_LEN), array('SPTAB_CHECK' ,'NUM_RANGE_CHECK'));
            $objErr->doFunc(array(t('c_Login ID_01'), 'login_id', ID_MIN_LEN, ID_MAX_LEN), array('SPTAB_CHECK' ,'NUM_RANGE_CHECK'));
        }
